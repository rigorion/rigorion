...
const SegmentedProgress = ({
  ...
  return <div className={cn("relative w-60 h-60 mx-auto", className)}>
      <svg className="w-full h-full -rotate-90">
        {createSegments().map((segment, index) => <path key={index} d={segment.path} className={cn("stroke-[12] fill-none transition-all duration-300", segment.isActive ? "stroke-[#4ade80]" : "stroke-gray-100")} />)}
      </svg>
