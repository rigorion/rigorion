...
interface StatsCardProps {
  stat: StatProps | { component: React.ComponentType };
  index: number;
}
...
export const StatsCard = ({ stat, index }: StatsCardProps) => {
  if ('component' in stat) {
    const Component = stat.component;
    return <Component />;
  }

  const { title, value, icon: Icon, color, shadowColor, fill = false } = stat;
  
  return (
    <Card className="p-4 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden group">
      <div className="flex items-center gap-3">
        <motion.div
          className={`p-2.5 rounded-full ${color} ${shadowColor} shadow-lg`}
...
export const StatsCardGrid = ({ stats }: { stats: Array<StatProps | { component: React.ComponentType }> }) => {
  return (
    <motion.div
