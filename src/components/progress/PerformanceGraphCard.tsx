...
export const PerformanceGraphCard = ({ data = [] }: PerformanceGraphCardProps) => {
  ...
  const calculateTrend = () => {
    if (data.length < 6) return { trend: 0, percentage: 0 };
    ...
    if (previousDays === 0) return { trend: 0, percentage: 0 };
  ...
  const averageQuestions = data.length > 0 
    ? Math.round(data.reduce((sum, item) => sum + item.attempted, 0) / data.length) 
    : 0;
