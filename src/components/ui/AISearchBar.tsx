...
const AISearchBar: React.FC<AISearchBarProps> = ({
  ...
  return (
    <div className={cn('relative w-full max-w-4xl mx-auto', className)}>
      ...
      <div className={cn(
        'relative flex flex-col gap-4 p-6 transition-all duration-500 ease-out',
        'bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl border',
        isFocused 
          ? 'shadow-2xl border-red-300/50 scale-[1.02]' 
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-4">
          <div className="relative flex-1">
            <Input
              ...
              className={cn(
                'border-2 bg-white h-14 px-6 rounded-2xl text-lg',
          ...
          <Button
            ...
            disabled={!query.trim()}
            className={cn(
              'rounded-2xl h-14 w-14 p-0 transition-all duration-300 ease-out',
              'hover:scale-110 transform-gpu disabled:scale-100 disabled:opacity-50',
              'flex items-center justify-center shadow-lg',
              'bg-gradient-to-r from-[#8A0303] to-red-600 hover:shadow-xl text-white'
            )}
          >
