                ...
                {PROMOTIONAL_ITEMS.map((item, index) => <CarouselItem key={item.id} className={`pl-4 transition-all duration-300 ${activeIndex === index ? 'scale-100 z-10' : 'scale-90 opacity-80'}`}>
                    <Dialog open={selectedItem?.id === item.id} onOpenChange={handleDialogClose}>
                      <DialogTrigger asChild>
                        <div className="h-full cursor-pointer" onClick={() => handleItemClick(item)}>
                          <div className="relative h-[550px] overflow-hidden rounded-xl">
                            ...
                            <div className="absolute bottom-4 right-4">
                              <span className={`px-3 py-1 text-xs font-medium rounded-full ${activeIndex === index ? 'bg-green-500 text-white' : 'bg-white/90 text-gray-800'}`}>
